...
var client;
var statusBarItem;
var defaultStatus = { state: "init", docCount: 0 };
var deadStatus = { state: "dead", docCount: 0 };
var extId = "marksman";
var extName = "Marksman";
var compatibleServerRelease = MARKSMAN_BIN_VERSION;
var releaseBaseUrl = "https://github.com/artempyanykh/marksman/releases/download";
...
async function activate(context) {
  if (!import_coc.workspace.getConfiguration("marksman").get("enable", true))
    return;
  statusBarItem = createDefaultStatus();
  statusBarItem.show();
  client = await connectToServer(context, statusBarItem);
  const restartServerCmd = import_coc.commands.registerCommand(`${extId}.restartServer`, async () => {
    stopClient(client);
  ...
  const showOutputCmd = import_coc.commands.registerCommand(`${extId}.showOutputChannel`, () => {
    if (client) {
...
async function connectToServer(context, status) {
  let serverOptions;
  const maybeServerOptions = await mkServerOptions(context);
...
async function mkServerOptions(context) {
...
function mkServerOptionsFromConfig() {
  const extConf = import_coc.workspace.getConfiguration(`${extId}`);
  const customCommand = extConf.get("customCommand");
  const customCommandDir = extConf.get("customCommandDir");
  if (customCommand) {
...
async function downloadRelease(targetDir, onProgress) {
  const targetFile = import_path.default.join(targetDir, serverBinName());
  const tempName = (Math.round(Math.random() * 100) + 1).toString();
  const tempFile = import_path.default.join(targetDir, tempName);
  const downloadUrl = releaseDownloadUrl();
  console.log(`Downloading from ${downloadUrl}; destination file ${tempFile}`);
  const resp = await lib_default(downloadUrl);
  if (!resp.ok) {
  ...
  const destStream = import_fs.default.createWriteStream(tempFile);
  const downloadProcess = (0, import_util.promisify)(import_stream2.default.pipeline);
  await downloadProcess(resp.body, destStream);
  console.log(`Downloaded the binary to ${tempFile}`);
  import_fs.default.renameSync(tempFile, targetFile);
  await import_fs.default.promises.chmod(targetFile, 493);
}
async function downloadServerFromGH(context) {
  const targetDir = import_path.default.join(context.storagePath, compatibleServerRelease);
...
async function findServerInPath() {
  const binName = serverBinName();
  const inPath = new Promise((resolve, reject) => {
    (0, import_which.default)(binName, (err, path2) => {
      if (err) {
        reject(err);
      }
      if (path2 === void 0) {
...
function createClient(serverOptions, clientOptions) {
  const client2 = new import_coc.LanguageClient(extId, extName, serverOptions, clientOptions);
  configureClient(client2);
  return client2;
}
function configureClient(client2) {
  client2.registerFeature(new ExperimentalFeatures());
  client2.onReady().then(() => {
    console.log("Client onReady");
    client2.onNotification(statusNotificationType, (statusParams) => {
      console.log("Got marksman/status notification");
      updateStatus(statusBarItem, statusParams);
    });
  });
  client2.onDidChangeState((ev) => {
    if (ev.newState === import_coc.State.Stopped) {
      updateStatus(statusBarItem, deadStatus);
    }
  });
}
function createDefaultStatus() {
  const item = import_coc.window.createStatusBarItem(99);
  updateStatus(item, defaultStatus);
  return item;
}
function updateStatus(item, statusParams) {
  let status;
...
async function stopClient(client2) {
  if (client2) {
    await client2.stop();
    client2.outputChannel.dispose();
  }
}
